...
def readFile(filename):
    ...
    #os.path.normpath()
    fs, channels = wavfile.read(PATH + filename)
...
def double_plot(signal, steps = None, simplify = True, outputType = None):
    ...
    for step in range(step_total):
        ...
        ax1.set_title(f'{signal.name}: Power spectral density at step {step:04d} ({datetime.timedelta(seconds=int(step*signal.timeStep))}) with each step = {signal.timeStep}s')
        ax1.set_ylabel('Amplitude (dB)')

        fig.savefig(f'{PATH}/outputs/images/{step:04d}.png', dpi=300)
        ax1.clear()

    if outputType == None:
        outputType = 'gif'
    os.system(f"ffmpeg -y -framerate {int(1/signal.timeStep)} -i {PATH}/outputs/images/%04d.png -i {PATH}/resources/palette.png -lavfi paletteuse {PATH}/outputs/animations/{signal.name}.{outputType}")

    if os.name == 'nt':
        os.system(f"del {os.path.normpath(PATH+'/outputs/images/*.png')}")
    else:
        os.system(f"rm {PATH}/outputs/images/*.png")
