...
class signal:
    ...
    def spectral(self, step=None, simplify=True):
        ...
        if (int(self.timeStep*step)%1 == 0):
            self.noise_offset = tools.calculate_offset(avg_mag)   
        avg_mag += self.noise_offset + safety_factor
        filtered_mag = np.clip(avg_mag, a_min=0., a_max=None)
        tools.channel_filter(filtered_mag, pass_step_width = int(self.pass_bandwidth / self.bandWidth * self.resolution))
...
class step:
    def __init__(self, step, fs, time_data, freq_band, resolution, bandwidth, pass_step_width):
        self.step = step
        self.fs = fs
        ...
        self.resolution = resolution
        self.bandwidth = bandwidth
        self.pass_step_width = pass_step_width

    def spectral_density(self, step=None, simplify=True):
        localSignal = self.rawSignal[int(self.fs * self.timeStep * step): int(self.fs * self.timeStep * (step+1))]/127.
        localSignal *= np.hanning(int(self.fs * self.timeStep))
        raw_mag = 20*np.log10(np.abs(np.fft.fft(localSignal)[self.bandwidthIndex]))
        safety_factor = 0
